...
const Comment = ({ post, setSelectedPost }) => {
  ...
  const handleSendComment = async () => {
    try {
      ...
      await axios.post(
        "https://cruise.pythonanywhere.com/annon/posts/comment/",
        data,
        { headers }
      );
      setInputValue("");
  ...
  return (
    <div className="z-50 h-screen flex flex-col pt-4 px-3">
      ...
      <div className="z-10 md:mx-10 p-3">
        ...
        />
        <Gist content={post.content} showFullGist={true} />
        <GistLinks post={post} />
      </div>
      <p className="px-3 my-3 text-gray-400">{post.comments.length} comments</p>
      <div className="mx-2  overflow-auto border-l border-gray-500">
      ...
      <div className="mt-auto w-[100%] border-t border-gray-500 left-0 bottom-0 py-2 px-2">
        ...
        />
        <button
          className="ml-3 rounded-xl font-bold text-sm"
